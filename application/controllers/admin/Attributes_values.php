...
class Attributes_values extends Admin_Controller {
	...
	$save['attribute_id'] = $this->input->post('attribute_id');
     ...
     function delete($attribute_value_id) {

        $this->attributes_model->delete_row($attribute_value_id);
        $this->session->set_flashdata('message', ('The product category has been deleted!'));
        redirect($this->config->item('admin_folder') . '/attributes_values/form');
    }
